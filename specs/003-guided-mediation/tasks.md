...
**US1 slice status (2026-04-18)**: transport verification +
session-open happy path shipped (previous slice) and reasoning-health
gating + SC-105 gating test shipped (this slice). `MostroChatSim` /
`MockReasoningProvider` / `UnhealthyReasoningProvider` remain inline
inside their respective `tests/phase3_session_open*.rs` files rather
than in `tests/common/mod.rs` (T024, T025 still deferred — promote to
shared fixtures when a third integration test needs them). The
auth-retry loop + its gates (T042, T043), the rationales +
mediation_events audit helpers (T032, T033), the policy / draft
extraction (T038, T039), and the engine loop wiring (T040) remain
deferred to later US1 continuation slices; US2–US5 phases are
unchanged.
