...
- [X] T019 Spawn the Phase 3 mediation background engine (`tokio::task`) from `src/daemon.rs` only after the startup health check passes. The engine task loop is implemented in `src/mediation/mod.rs::run(...)` and is wired here with a shutdown channel tied into the existing shutdown path
