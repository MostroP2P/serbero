...
- [X] T021 [US2] Integration test `tests/phase4_supersession.rs`. Sub-tests:
  ...
  - `dispute_resolving_after_dispatch_does_not_recall` (substituted) — the originally-specified sub-test was `dispute_resolving_between_scan_and_send_still_dispatches`, which would require a deterministic seam inside `process_one` between `dispute_metadata` and `send_to_recipients` so the test could flip `lifecycle_state` mid-flight. Exposing that seam for tests would leak the worker's internal staging into the public surface of `src/escalation/mod.rs`, so we pin an adjacent property instead: once `send_to_recipients` has completed and `record_successful_dispatch` has written the `escalation_dispatches` row, a subsequent flip to `lifecycle_state = 'resolved'` must NOT retract the dispatch and must NOT emit a supersession audit row on the next cycle (the handoff is consumed by the dispatch row, so the consumer scan filters it). This still honors US2 acceptance scenario 2's "Phase 4 does NOT attempt to recall" at the stronger granularity of "a completed dispatch is terminal".
  - `supersession_does_not_mark_handoff_consumed` — after a supersession, the handoff row stays available for a future scan (consumer still sees it as pending because no `escalation_dispatches` row was written). The sub-test first asserts the audit is **idempotent per `handoff_event_id`**: cycle 1 writes supersession row #1, cycle 2 against the still-resolved dispute writes NO new row (count stays at 1). Dedup lives at the writer (`tracker::record_supersession`), not at the scan's `NOT EXISTS` clause — filtering at the scan would hide the handoff from the gate on cycle N+1 and break FR-213. It then pins FR-213 directly: flip `lifecycle_state` back to `notified` (policy-change simulation) and run another cycle; the previously-superseded handoff is now dispatched through the normal send path, proving the handoff was never consumed from the scan's perspective. The historical supersession row is preserved so the audit trail documents both states.
...
- [X] T022 [US3] Handle `Recipients::Unroutable` inside `src/escalation/mod.rs::run_dispatcher`: call `tracker::record_unroutable` (new helper, T023), emit one `error!` log line naming the dispute id plus the config hint `fallback_to_all_solvers`. Do NOT write an `escalation_dispatches` row; the handoff stays pending so a future config change can pick it up (FR-213).

- [X] T023 [US3] Add `pub async fn record_unroutable(conn, handoff: &PendingHandoff, configured_solver_count: usize) -> Result<()>` to `src/escalation/tracker.rs`. Calls `db::mediation_events::record_escalation_dispatch_unroutable` with the payload shape from `contracts/audit-events.md`. Implemented with a writer-side idempotence check (`escalation_dispatch_unroutable_exists_for_handoff`) mirroring the US2 supersession pattern — a stuck-unroutable handoff does not grow the audit table by one row per cycle; a future config change that adds a Write solver still re-picks the handoff because the scan is untouched (FR-213).

- [X] T024 [US3] Extend the dispatch wiring in `src/escalation/mod.rs` (T016) so the `Recipients::Broadcast { via_fallback: true }` arm sets `fallback_broadcast = true` when calling `tracker::record_successful_dispatch`. Audit-event payload's `fallback_broadcast` field flows through T015. (Wiring was already present from T016 — `process_one` destructures `via_fallback` out of `Recipients::Broadcast` and passes it verbatim as the `fallback_broadcast` parameter of `record_successful_dispatch`. T025's rule-3 sub-test pins the end-to-end flow through both the DB column and the audit payload.)

- [X] T025 [US3] Integration test `tests/phase4_no_write_solvers.rs`. Sub-tests:
  - `zero_write_solvers_fallback_off_records_unroutable` — config has only Read solvers, `fallback_to_all_solvers = false`, one handoff → zero DMs, zero dispatch rows, exactly one `escalation_dispatch_unroutable` audit row. (The integration test asserts the payload shape — `handoff_event_id`, `configured_solver_count`, `fallback_to_all_solvers` — rather than snooping on tracing output; the ERROR log line itself is covered by the one-cycle vs two-cycle idempotence assertion that follows.) A second cycle with the same broken config keeps the count at 1 (writer-side dedup).
  - `zero_write_solvers_fallback_on_broadcasts_to_everyone` — same config but `fallback_to_all_solvers = true` → DMs go to every configured Read solver, one dispatch row with `fallback_broadcast = 1` and `target_solver` = comma-joined list of every configured pubkey, the audit payload's `fallback_broadcast` key is `true`, and NO `escalation_dispatch_unroutable` row is written (rule 3 wins over rule 4).
  - `unroutable_handoff_picked_up_after_config_change` — run one cycle with fallback off and zero Write solvers (produces the unroutable row); swap the config to add a Write solver; run another cycle on the same DB; the handoff now dispatches normally (`status = 'dispatched'`, `fallback_broadcast = 0`, `target_solver` = the newly-added Write solver's pubkey). The pre-existing unroutable audit row is preserved so history documents the broken-config interval. Pins FR-213's "stays unconsumed" guarantee.
