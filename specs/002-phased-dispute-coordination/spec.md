...
**Created**: 2026-04-16
**Status**: Draft
**Input**: User description: "Phased specification for Serbero — dispute coordination, notification, and assistance system for the Mostro ecosystem, evolving from always-on dispute listener through guided mediation and escalation support"

## System Vision

Serbero is a dispute coordination system for the Mostro ecosystem.
...
Serbero evolves in phases. Each phase adds a bounded set of
...
- Serbero MUST NOT move funds, sign `admin-settle` or
  `admin-cancel`, or close disputes.
- Mostro owns escrow state, permissions, solver roles, and
  dispute-closing authority.
- Serbero owns monitoring, notification, coordination, assistance,
...
- Q: How should Serbero behave when SQLite persistence fails during deduplication? → A: Serbero halts notifications when SQLite is unreadable and resumes when persistence recovers. Deduplication integrity is prioritized over delivery.
...
- **TC-003**: Serbero MUST use SQLite as its internal database.
...
receives an encrypted Nostr direct message from Serbero informing
...
Serbero. Without reliable, prompt notification, disputes go
...
2. **Given** Serbero has already notified solvers about a dispute,
...
3. **Given** Serbero is restarted after a crash or planned
   restart,
...
4. **Given** a relay disconnects unexpectedly,
   **When** Serbero detects the disconnection,
...
Serbero tracks it as "new," then simulating a solver taking the
dispute in Mostro and verifying Serbero transitions its state to
"taken" and suppresses further notifications for that dispute.

**Acceptance Scenarios**:

1. **Given** Serbero has detected and notified solvers about a
   dispute,
   **When** Serbero sends a re-notification or assignment update,
...
3. **Given** a dispute has been taken by a solver,
   **When** Serbero detects the assignment,
   **Then** Serbero sends an assignment notification to all
...
4. **Given** a dispute remains in "notified" state beyond a
   ...
   **Then** Serbero re-notifies solvers with a message indicating
...
may be delayed or confused about the process. Serbero contacts
...
Serbero identifies itself as an assistance system, not the final
...
both parties respond cooperatively, verifying Serbero sends
...
1. **Given** a dispute is classified as a potential coordination
   ...
   **When** Serbero begins guided mediation,
...
4. **Given** information is incomplete or unclear during mediation,
   **When** Serbero generates messages or summaries,
...
claimed, what Serbero attempted, and why it is escalating.
...
A system administrator deploys Serbero with a direct API-based
...
changes to Serbero's core policy, notification, or escalation
logic.
...
verifying that Serbero's policy layer produces consistent behavior
regardless of which backend generated the classification.

**Acceptance Scenarios**:

1. **Given** Serbero is configured with a reasoning backend,
...
- What happens when the Nostr relay is unreachable during
  notification? Serbero retries reconnection with backoff and
...
- What happens when Serbero restarts and there are disputes it
  ...
  Serbero checks its SQLite state against fresh relay events and
...
- What happens when two solvers attempt to take the same dispute
  simultaneously? Mostro enforces the assignment. Serbero
  observes whichever assignment event arrives and updates state
  accordingly.

- What happens when no solvers are configured? Serbero logs a
...
- Serbero MUST run as a long-lived daemon process.
...
- Serbero MUST use SQLite directly. No storage abstraction layer.
...
- If a dispute remains in "notified" state beyond a configurable
  timeout (default: 5 minutes), Serbero MUST re-notify all
...
- Serbero MUST surface dispute status passively: through
...
- Solvers learn dispute status from the notifications they receive,
  not by querying Serbero directly.
...
- Serbero MUST contact dispute parties via encrypted gift-wrap
  messages.
- All messages MUST identify Serbero as an assistance system, not
  the final authority.
- Serbero MUST ask targeted clarifying questions relevant to the
  dispute category.
- Serbero MUST communicate uncertainty honestly and MUST NOT
  fabricate evidence or imply certainty it does not have.

#### Mediation Outcomes

- If both parties respond cooperatively and facts align, Serbero
...
- If mediation stalls (unresponsive parties, conflicting claims,
  or low confidence), Serbero escalates to a human operator.
...
**Scope**: Defines how Serbero escalates cases that MUST NOT
remain in assisted flow to human write operators.

#### Escalation Triggers

Serbero MUST escalate when:
...
- If no write-permission solver acknowledges within a configurable
  timeout, Serbero MUST re-escalate with increased urgency.
- Once escalated, Serbero MUST cease autonomous mediation on
  that dispute.
...
- Serbero's policy layer owns all decisions about escalation
...
- If the reasoning backend is unavailable, Serbero MUST fall back
  to immediate operator escalation.
- If the reasoning backend returns low confidence, Serbero MUST
...
- **FR-001**: Serbero MUST run as a long-lived daemon that
  maintains persistent connections to configured Nostr relay(s).

- **FR-002**: Serbero MUST subscribe to dispute events scoped to
  the configured Mostro instance and detect new disputes with status
  `initiated`.

- **FR-003**: Serbero MUST send encrypted gift-wrap notifications
  to all registered solvers when a new dispute is detected (Phase 1).

- **FR-004**: Serbero MUST persist detected dispute records and
  notification attempts in SQLite to prevent duplicate notifications
  across restarts and relay replays.

- **FR-005**: Serbero MUST attempt reconnection with backoff when
  relay connections drop, and MUST NOT crash on relay disconnection.

- **FR-006**: Serbero MUST log notification failures in SQLite for
  observability.

- **FR-007**: Serbero MUST track dispute lifecycle states (new,
  notified, taken, waiting, escalated, resolved) starting in Phase 2.

- **FR-008**: Serbero MUST detect solver assignment events from
  Mostro and transition disputes to "taken" state (Phase 2).

- **FR-009**: Serbero MUST re-notify solvers when disputes remain
  unattended beyond a configurable timeout (Phase 2).

- **FR-010**: Serbero MUST communicate with dispute parties via
  encrypted gift-wrap messages during guided mediation (Phase 3).

- **FR-011**: Serbero MUST identify itself as an assistance system
  in all user-facing messages and MUST NOT present itself as the
  final dispute authority (Phase 3+).

- **FR-012**: Serbero MUST escalate to write-permission solvers
...
- **FR-014**: Serbero MUST route reasoning requests through a
  defined backend interface and independently validate all reasoning
  output before acting (Phase 5).

- **FR-015**: Serbero MUST fall back to immediate operator
  escalation when the reasoning backend is unavailable (Phase 5).

- **FR-016**: Serbero MUST NOT execute or sign `admin-settle`,
...
- **FR-019**: Serbero MUST use SQLite directly without a storage
  abstraction layer in all initial phases.

### Key Entities

- **Dispute**: An active dispute detected by Serbero. Key
...
- **Solver**: A human dispute resolver registered with Serbero.
...
- **AuditEntry**: A log of any Serbero action. Key attributes:
...
- **SC-003**: After a restart, Serbero correctly identifies
...
- **SC-006**: Mostro continues to operate normally when Serbero
...
- **SC-011**: Serbero never presents itself as the final authority
...
- **SC-013**: Escalation summaries include all required fields
  ...
  dispute without requesting additional context from Serbero.
...
| Failure                        | Behavior                                                  |
|--------------------------------|-----------------------------------------------------------|
| All relays unreachable         | Serbero retries reconnection; logs degraded mode       |
...
| Serbero fully offline       | Mostro operates normally; solvers resolve manually        |

## Assumptions

- Mostro publishes dispute events to Nostr relays that Serbero
...
- The solver list (public keys and optionally permission levels) is
  provided to Serbero via static configuration.
...
- Mostro dispute events are kind 38386 addressable events with tags:
  ...
  (dispute). These tags provide sufficient metadata for Serbero
...
- Serbero may suggest outcomes to solvers, but solvers act through
  Mostro, not through Serbero.
- If Serbero is offline, Mostro and its operators continue to
