...
- [ ] T013 [P] Implement `src/nostr/subscriptions.rs::phase1_filter(mostro_pubkey: &str) -> Filter`: `Kind::Custom(38386)`, filter by the configured Mostro pubkey as the event `pubkey`, plus custom tag filters for `#z=dispute` and `#s=initiated` (treat `y` only as platform / optional instance-name metadata, not as a pubkey-bearing tag; use the SDK's author and single-letter-tag filter APIs confirmed in R-001 verification)
...
- [ ] T040 [US2] Extend `src/nostr/subscriptions.rs` with `phase2_filters(mostro_pubkey)` that additionally subscribes to kind 38386 events with `s=in-progress`, still filtering by the configured Mostro pubkey as the event `pubkey` rather than by `y`; update `src/daemon.rs` to use the Phase 2 filter set once US2 is active
