...
- [ ] T013 [P] Implement `src/nostr/subscriptions.rs::phase1_filter(mostro_pubkey: &str) -> Filter`: `Kind::Custom(38386)`, custom tag filters for `#z=dispute`, `#y=<mostro_pubkey>`, `#s=initiated` (use the SDK's single-letter-tag filter API confirmed in R-001 verification)
...
- [ ] T040 [US2] Extend `src/nostr/subscriptions.rs` with `phase2_filters(mostro_pubkey)` that additionally subscribes to kind 38386 events with `s=in-progress` (scoped to `#y=<mostro_pubkey>`); update `src/daemon.rs` to use the Phase 2 filter set once US2 is active
